...
export async function setDisplayName(displayName) {
  ...
  if (error) {
    const hint = error.message?.includes("set_my_display_name")
      ? " Uruchom Docs/supabase-profiles-username.sql w Supabase."
      : "";
...
export async function fetchMyDisplayName() {
  ...
  if (!user) {
    return { ok: false, error: authError, displayName: null };
  }
  ...
  if (error) {
    return { ok: false, error: error.message, displayName: null };
  }

  return { ok: true, displayName: data?.display_name ?? null };
}

const DISPLAY_NAME_STORAGE_KEY = "your-soul-pending-display-name";

export function storePendingDisplayName(name) {
  if (typeof window === "undefined" || !name?.trim()) return;
  sessionStorage.setItem(DISPLAY_NAME_STORAGE_KEY, name.trim());
}

export function readPendingDisplayName() {
...
export async function applyPendingDisplayNameIfAny() {
  const pending = readPendingDisplayName();
  if (!pending) {
  ...
  if (result.ok) {
    ...
    return { ok: true, applied: true, displayName: result.displayName };
  }
...
export async function fetchMySavedData() {
  ...
  const [profileRes, collectionsRes, marginsRes, reactionsRes] =
    await Promise.all([
      supabase
        .from("profiles")
        .select("worlds, odcisk, display_name, updated_at")
        .eq("id", user.id)
        .maybeSingle(),
      supabase
        .from("collections")
      ...
      supabase
        .from("margin_reactions")
        .select(
          "id, margin_id, created_at, margins(body, quotes(text, author))"
        )
        .eq("user_id", user.id)
        .order("created_at", { ascending: false }),
    ]);
...
export async function fetchConstellationData() {
  ...
  let warning = marginsRes.error?.message ?? null;
  ...
  if (niciWarning) {
    warning = warning ? `${warning} · ${niciWarning}` : niciWarning;
  }
...
export async function fetchZielnikData() {
  ...
  const [collectionsResult, marginsRes, niciRes] = await Promise.all([
    ...
    supabase
      .from("margins")
      .select("quote_id, body, visibility, created_at")
      .eq("user_id", user.id),
    supabase
      .from("nici")
...
export async function fetchMatchCards(mode = "like_me") {
  const supabase = getSupabaseBrowserClient();
  const { user, error: authError } = await getCurrentUser(supabase);

  if (!user) {
    return { ok: false, error: authError, cards: [], myOdcisk: null };
  }
  ...
  const [profileRes, cardsRes] = await Promise.all([
    supabase
      ...
      .eq("id", user.id)
      .maybeSingle(),
    supabase.rpc("get_my_match_cards", { p_mode: mode }),
  ]);

  if (profileRes.error) {
    return { ok: false, error: profileRes.error.message, cards: [], myOdcisk: null };
  }

  if (cardsRes.error) {
    const hint =
      cardsRes.error.message?.includes("get_my_match_cards")
        ? " Uruchom Docs/supabase-matches.sql w Supabase SQL Editor."
        : "";
    return {
      ok: false,
      error: `${cardsRes.error.message}${hint}`,
      cards: [],
      myOdcisk: profileRes.data?.odcisk ?? null,
    };
  }

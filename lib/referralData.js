...
function normalizeInviteCodeRows(data) {
...
export async function fetchFirstAvailableInviteCode() {
  const result = await fetchMyInviteCodes();
  if (!result.ok) {
    return null;
  }
  const available = result.codes?.find((row) => row.status === "available");
  return available?.code ?? result.codes?.[0]?.code ?? null;
}
...
export async function fetchMyInviteCodes() {
  ...
  if (!error) {
    return { ok: true, codes: normalizeInviteCodeRows(data) };
  }
  ...
  if (rpcError) {
    return {
      ok: false,
      error: `${error.message} (${rpcError.message})`,
      codes: [],
    };
  }

  return { ok: true, codes: normalizeInviteCodeRows(rpcData) };
}
...
export async function validateInviteCode(code) {
  ...
  if (!normalized) {
    return { ok: false, valid: false, error: "Kod zaproszenia jest wymagany." };
  }
  ...
  if (error) {
    const hint = error.message?.includes("validate_invite_code")
      ? " Uruchom ponownie Docs/supabase-referrals.sql w Supabase."
      : "";
    return { ok: false, valid: false, error: `${error.message}${hint}` };
  }
  ...
  if (!data) {
    return {
      ok: true,
      valid: false,
      error: "Kod niedostępny lub już wykorzystany.",
    };
  }
